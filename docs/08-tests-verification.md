...
| -------- | ----------------------------------------------------------- | ------------------------------ | -------- |
| INT-001  | `backends` retourne statuts cohérents (incl. `unsupported`) | CLI + adapters (`isAvailable`) | Haute    |
...
| INT-005  | `loop` timeout → code 75                                    | runner + timeout               | Haute    |
| INT-006  | `loop` maxIterations → code 4                               | runner                         | Haute    |
| INT-006b | `loop` no-progress → code 5                                 | runner + similarity            | Haute    |
| INT-006c | `loop` invalid-json (mode json) → code 65                   | runner + completion            | Haute    |
| INT-007  | `--artifacts` écrit dossier + fichiers                      | runner + fs                    | Moyenne  |
| INT-007b | `--artifacts` échoue à écrire → code 73                     | runner + fs                    | Moyenne  |
